...
python -m scripts.core_model.LULUCF_fluxes -cn LULUCF_model -bb 10 49.75 10.25 50 -cs 0.25 -yr 2015 2023
python -m scripts.core_model.LULUCF_fluxes -cn LULUCF_model -bb 115.25 -3.75 115.5 -3.5 -cs 0.25 --no_upload -yr 2015 2023
python -m scripts.core_model.LULUCF_fluxes -cn LULUCF_model -bb 10 49 11 50 -cs 1 --no_upload -yr 2015 2023
python -m scripts.core_model.LULUCF_fluxes -cn LULUCF_model -cshp s3://gfw2-data/climate/AFOLU_flux_model/fishnet_1x1deg/20241125/ -f 1 -yr 2015 2023
        ...
        for row in range(LC_curr_block.shape[0]):
            for col in range(LC_curr_block.shape[1]):
                ...
                if interval_type == cn.intervals_annual or interval_type == cn.intervals_five_years:
                        ...
                        if all_planted_trees:  # New planted trees (11)
                            ...
                            if all_oil_palm:  # New oil palm (incl. SDPT) (111)
                                ...
                                (c_gross_emis_out, c_gross_removals_out, c_dens_out, gain_year_count, forest_age_annual_cell) = (
                                    nu.calc_NT_T(interval_type, RF_AGC_final, RF_BGC_final, c_dens_in, deadwood_c_ratio=0, litter_c_ratio=0))
                            ...
                            if tall_veg_curr:  # New terrestrial natural forest (121)
                                ...
                                RF_BGC_final = RF_AGC_final * r_s_ratio_cell
                                (c_gross_emis_out, c_gross_removals_out, c_dens_out, gain_year_count, forest_age_annual_cell) = (
                                    nu.calc_NT_T(interval_type, RF_AGC_final, RF_BGC_final, c_dens_in, deadwood_c_ratio=deadwood_c_ratio, litter_c_ratio=litter_c_ratio))
                        ...
                        if all_planted_trees:  # Full loss of planted trees (21)
                            ...
                            if tall_veg_prev:  # Full loss of natural forest (221)
                                ...
                                if LC_curr == cn.cropland:  # Natural forest converted to cropland (2211->22111/22112)
                                    ...
                                    bgc_rf_in = agc_rf_in * r_s_ratio_cell
                                    rf_post_dist = np.array([cn.cropland_rf, 0, 0, 0]).astype('float32')
                    ...
                    #TODO Include emissions if it's natural forest converted to oil palm according to Descals planting year. Not currently including emissions.
                    elif (tree_prev) and (tree_curr):  # Trees remaining trees (3)    ##TODO: Include mangrove exception.
                        node = nu.accrete_node(node, 3)
                        if part_or_full_dist_in_prev_interval:  # Trees partially disturbed in the last interval (31)
                            node = nu.accrete_node(node, 1)
                            if all_planted_trees:   # Planted trees partially disturbed in the last interval (311)
                                node = nu.accrete_node(node, 1)
                                if sig_height_gain_prev_curr_abs:  # Oil palm/planted trees partially disturbed in the last interval with signif. height increase after (3111)
                                    # NOTE: This should only occur with 5-year interval data, not annual data.  #TODO Test with 5-year data
                                    node = nu.accrete_node(node, 1)
                                    ...
                                    if all_oil_palm:  # Oil palm partially disturbed in the last interval with signif. height increase after (31111->311111/311112)
                                        node = nu.accrete_node(node, 1)
                                        agc_rf_in = cn.oil_palm_agc_rf
                                        bgc_rf_in = cn.oil_palm_bgc_rf
                                        ...
                                        (state_out, c_gross_emis_out, c_gross_removals_out, non_co2_flux_out, c_dens_out,
                                         RF_AGC_final, RF_BGC_final, gain_year_count, forest_age_annual_cell) = nu.calc_T_T_non_stand_disturbs(
                                            ...
                                            c_pools_fire_CO2, c_pools_fire_non_CO2,
                                            c_pools_no_fire, forest_dist_last, interval_end_year, c_dens_in,
                                    ...
                                    else: # Planted trees partially disturbed in the last interval with signif. height increase after (31112->311121/311122)
                                        node = nu.accrete_node(node, 2)
                                        agc_rf_in = planted_forest_AGC_RF_cell
                                        bgc_rf_in = planted_forest_BGC_RF_cell
                                        rf_post_dist = np.array([agc_rf_in, bgc_rf_in, 0, 0]).astype('float32')
                                        ...
                                        (state_out, c_gross_emis_out, c_gross_removals_out, non_co2_flux_out, c_dens_out,
                                         RF_AGC_final, RF_BGC_final, gain_year_count, forest_age_annual_cell) = nu.calc_T_T_non_stand_disturbs(
                                            ...
                                            rf_post_dist, most_recent_year_not_tall_veg,
                                            Cf, Gef_co2_forest, Gef_ch4_forest, Gef_n2o_forest, deadwood_c_ratio=0, litter_c_ratio=0)
                                else:  # Oil palm/planted trees partially disturbed in the last interval without signif. height increase after (3112)
                                    # NOTE: All annual interval data is expected to use this branch.
                                    node = nu.accrete_node(node, 2)
                                    # Calculation function only uses the RFs for 5-year intervals but assigning them regardless of interval type for consistency
                                    if all_oil_palm: # Oil palm partially disturbed in the last interval without signif. height increase after (31121->311211/311212)
                                        node = nu.accrete_node(node, 1)
                                        agc_rf_in = cn.oil_palm_agc_rf
                                        bgc_rf_in = cn.oil_palm_bgc_rf
                                        rf_post_dist = np.array([0, 0, 0, 0]).astype('float32')
                                        ...
                                        (state_out, c_gross_emis_out, c_gross_removals_out, non_co2_flux_out, c_dens_out,
                                         RF_AGC_final, RF_BGC_final, gain_year_count, forest_age_annual_cell) = nu.calc_T_T_non_stand_disturbs(
                                            ...
                                            rf_post_dist, most_recent_year_not_tall_veg,
                                            Cf, Gef_co2_forest, Gef_ch4_forest, Gef_n2o_forest, deadwood_c_ratio=0, litter_c_ratio=0)
                                    else: # Planted trees partially disturbed in the last interval without signif. height increase after (31122->311221/311222)
                                        node = nu.accrete_node(node, 2)
                                        agc_rf_in = planted_forest_AGC_RF_cell
                                        bgc_rf_in = planted_forest_BGC_RF_cell
                                        rf_post_dist = np.array([0, 0, 0, 0]).astype('float32')
                            ...
                            node = nu.accrete_node(node, 2)
                            if all_planted_trees:  # Oil palm/planted trees not disturbed in the last interval (321)
                                node = nu.accrete_node(node, 1)
                                # Calculation function only uses the RFs for 5-year intervals but assigning them regardless of interval type for consistency
                                if all_oil_palm:  # Oil palm not disturbed in the last interval (3211->32111/32112)
                                    node = nu.accrete_node(node, 1)
                                    RF_AGC_final = cn.oil_palm_agc_rf
                                    RF_BGC_final = cn.oil_palm_bgc_rf
                                    c_pools_fire_CO2 = cn.agc_emissions_only
                                    ...
                                    state_out, c_gross_emis_out, c_gross_removals_out, non_co2_flux_out, c_dens_out, gain_year_count, forest_age_annual_cell = nu.calc_T_T_no_disturbs(
                                        node, interval_type, forest_age_annual_cell,
                                        ...
                                        Cf, Gef_co2_forest, Gef_ch4_forest, Gef_n2o_forest, deadwood_c_ratio=0, litter_c_ratio=0)
                                else: # Planted trees not disturbed in the last interval (3212->32121/32122)
                                    node = nu.accrete_node(node, 2)
                                    RF_AGC_final = planted_forest_AGC_RF_cell
                                    RF_BGC_final = planted_forest_BGC_RF_cell
                                    c_pools_fire_CO2 = cn.agc_emissions_only
                                ...
                                if tall_veg_curr:  # Forest not disturbed in last interval (3221)
                                    ...
                                    if primary_forest_proxy:  # >100 year old natural forest not disturbed in last interval (32211->322111/322112)
                                        ...
                                        state_out, c_gross_emis_out, c_gross_removals_out, non_co2_flux_out, c_dens_out, gain_year_count, forest_age_annual_cell = nu.calc_T_T_no_disturbs(
                                            ...
                                            Cf, Gef_co2_forest, Gef_ch4_forest, Gef_n2o_forest,
                                            deadwood_c_ratio=deadwood_c_ratio, litter_c_ratio=litter_c_ratio)
                                    else: # <100 year old natural forest not disturbed in last interval (32212->322121/322122)
                                        node = nu.accrete_node(node, 2)
...
def calculate_and_upload_LULUCF_fluxes(bounds, primary_forest_RFs, download_dict_with_data_types,
    ...
    print(f"At start of task {bounds_str}: {process.memory_info().rss / 1024 ** 2:.2f} MB")
    ...
    print(f"After downloading inputs for {bounds_str}: {process.memory_info().rss / 1024 ** 2:.2f} MB")
    ...
    print(f"After creating typed dicts for {bounds_str}: {process.memory_info().rss / 1024 ** 2:.2f} MB")

    updated_download_dict.clear()
    del updated_download_dict
    del futures
    gc.collect()
    print(f"After deleting updated_download_dict and futures for {bounds_str}: {process.memory_info().rss / 1024 ** 2:.2f} MB")
    ...
    print(f"Before numba calculations typed dicts for {bounds_str}: {process.memory_info().rss / 1024 ** 2:.2f} MB")
    ...
    print(f"After numba calculations typed dicts for {bounds_str}: {process.memory_info().rss / 1024 ** 2:.2f} MB")
    ...
    print(f"After creating combined output dict for {bounds_str}: {process.memory_info().rss / 1024 ** 2:.2f} MB")

    gc.collect()

    print(f"After gc.collect() for {bounds_str}: {process.memory_info().rss / 1024 ** 2:.2f} MB")
    ...
    print(f"After deleting input data for {bounds_str}: {process.memory_info().rss / 1024 ** 2:.2f} MB")
    ...
    print(f"Before creating pixel area chunk for {bounds_str}: {process.memory_info().rss / 1024 ** 2:.2f} MB")
    ...
    print(f"After creating pixel area chunk for {bounds_str}: {process.memory_info().rss / 1024 ** 2:.2f} MB")
    ...
    print(f"After populating output chunk stats for {bounds_str}: {process.memory_info().rss / 1024 ** 2:.2f} MB")
        ...
        if isinstance(v, dict):
            ...
            for key, val in v.items():
                if isinstance(val, np.ndarray):
                    print(f"  {key}: {val.nbytes / 1024 ** 2:.2f} MB")
    ...
    print(f"Before saving and uploading for {bounds_str}: {process.memory_info().rss / 1024 ** 2:.2f} MB")
    ...
    if not no_upload:
        ...
        print(f"Before uploading for {bounds_str}: {process.memory_info().rss / 1024 ** 2:.2f} MB")
        ...
        print(f"After uploading for {bounds_str}: {process.memory_info().rss / 1024 ** 2:.2f} MB")

    # Clears memory of unneeded arrays
    # Explicitly free output arrays after upload
    out_dict_all_dtypes.clear()
    del out_dict_all_dtypes
    gc.collect()
    print(f"After deleting out_dict_all_dtypes for {bounds_str}: {process.memory_info().rss / 1024 ** 2:.2f} MB")
...
def main(cluster_name, year_range, run_local=False, no_stats=False, no_log=False, no_aggregate=False, no_upload=False,
    ...
    if interval_type == cn.intervals_five_years:
        ...
        # download_dict[cn.soil_c_2000_pattern] = f"{sample_tile_id}_{cn.soil_c_2000_pattern}.tif"
