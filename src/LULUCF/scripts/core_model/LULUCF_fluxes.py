                        ...
                        if tall_veg_prev:  # Full loss of natural forest (221)
                            ...
                            if LC_curr == cn.cropland:  # Natural forest converted to cropland (2211)
                                ...
                                if burned_in_last_interval:  # Natural forest converted to cropland (burned) (22111)
                                    ...
                                    ef = cn.all_non_soil_pools
                                    agc_rf_post = 4.7
                                    c_gross_emis_out, c_gross_removals_out, non_co2_flux_out, c_dens_out, gain_year_count = nu.calc_T_NT(agc_rf_pre, ef, forest_dist_last, r_s_ratio_cell, interval_end_year, c_dens_in, 0, 0, 0)
...
def main(cluster_name, bounding_box, chunk_size, run_local=False, no_stats=False, no_log=False, no_upload=False):
    ...
    compile_and_upload_log_local(no_log, client, cluster, stage, len(chunks), chunk_size, start_time, end_time_2,
                              success_count, skipping_chunk_count, bounding_box, log_note)
...
def compile_and_upload_log_local(no_log, client, cluster, stage, chunk_count, chunk_size_deg,
    ...
    end_time_message = f"Stage {stage} ended at: {end_time_str}"
