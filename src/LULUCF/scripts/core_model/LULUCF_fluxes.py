...
@jit(nopython=True)
def calc_T_T_non_stand_disturbs_local(node, burned_in_last_interval, agc_rf, bgc_rf, c_pools_fire_CO2, c_pools_fire_non_CO2, c_pools_no_fire,
                    forest_dist_last, interval_end_year, c_dens_in,
                    post_dist_RF, most_recent_year_not_tall_veg, Cf, Gef_ch4, Gef_n2o,
    ...
    if burned_in_last_interval:
        agc_ef_CO2, bgc_ef_CO2, deadwood_c_ef_CO2, litter_c_ef_CO2 = nu.unpack_emission_factors(c_pools_fire_CO2)
    else:
    ...
    # Step 3: Calculates gross removals by carbon pools before disturbance. Gross removals are negative.
    ...
    # Step 6: Updates gross removals to include post-disturbance gross removals, if applicable (>=5 m height gain).
    ...
    if burned_in_last_interval:
        ...
        ch4_flux_out, n2o_flux_out = nu.non_CO2_fire_equations(c_pools_for_fire_total, Cf, Gef_ch4, Gef_n2o)
...
def LULUCF_fluxes(in_dict_uint8, in_dict_int16, in_dict_int32, in_dict_float32, primary_forest_RFs, is_final):
    ...
    planted_forest_AGC_removal_factor_block = in_dict_float32[cn.planted_forest_AGC_removal_factor_pattern]
    planted_forest_AGC_BGC_removal_factor_block = in_dict_float32[cn.planted_forest_AGC_BGC_removal_factor_pattern]
    ...
    for interval_end_year in list(range(cn.first_model_year, cn.last_model_year + 1, cn.interval_years))[1:]:
        ...
        for row in range(LC_curr_block.shape[0]):
            for col in range(LC_curr_block.shape[1]):
                ...
                natrl_forest_curve_16_20_cell = natrl_forest_curve_16_20_block[row, col]
                natrl_forest_curve_21_100_cell = natrl_forest_curve_21_100_block[row, col]
                ...
                planted_forest_AGC_removal_factor_cell = planted_forest_AGC_removal_factor_block[row, col]
                planted_forest_AGC_BGC_removal_factor_cell = planted_forest_AGC_BGC_removal_factor_block[row, col]
                planted_forest_BGC_removal_factor_cell = planted_forest_AGC_BGC_removal_factor_cell - planted_forest_AGC_removal_factor_cell
                ...
                # Removal factor for primary forests/IFL (Mg AGC/ha/yr)
                primary_forest_RF = nu.calc_primary_forest_RF(continent_ecozone_cell, primary_forest_RFs)
                ...
                # primary forest/IFL RF is used if in that, otheerwise old secondary forest RF is used.
                ...
                if interval_end_year == 2005:   # If in first interval...
                    if ifl_primary_cell:   # And if in IFL/primary forest...
                        agc_rf_pre_dist_prev = primary_forest_RF   # Use the primary forest/IFL RF
                    else:  #If not in IFL/primary forest...
                        agc_rf_pre_dist_prev = natrl_forest_curve_21_100_cell   # Use old secondary forest RF
                else:  # If not in first interval...
                ...
                # Cf for fire emissions for all gases for forests specifically (g/kg dry matter)
                ...
                ### Trees remaining trees
                    ...
                    if (forest_dist_last > 0) or (sig_height_loss_prev_curr_abs) or (first_time_sig_loss_from_max_height == 1):  # Trees partially disturbed in the last interval (31)
                        node = nu.accrete_node(node, 1)
                        if all_planted_trees:   # Planted trees partially disturbed in the last interval with signif. height increase after (311)
                            node = nu.accrete_node(node, 1)
                            if sig_height_gain_prev_curr_abs:  # Planted trees partially disturbed in the last interval with signif. height increase after (3111->31111/31112)
                                ...
                                state_out, c_gross_emis_out, c_gross_removals_out, non_co2_flux_out, c_dens_out, gain_year_count = calc_T_T_non_stand_disturbs_local(
                                    node, burned_in_last_interval, agc_rf, bgc_rf, c_pools_fire_CO2, c_pools_fire_non_CO2,
                                    c_pools_no_fire, forest_dist_last, interval_end_year, c_dens_in,
                                    rf_post_dist, most_recent_year_not_tall_veg, Cf, Gef_ch4_forest, Gef_n2o_forest, deadwood_c_ratio=0, litter_c_ratio=0)
                    ...
                    #                 c_pools_no_fire = cn.biomass_emissions_only
                    #                 state_out, c_gross_emis_out, c_gross_removals_out, non_co2_flux_out, c_dens_out, gain_year_count = calc_T_T_non_stand_disturbs_local(
                    #                     node, burned_in_last_interval, agc_rf, bgc_rf, c_pools_fire_CO2,
                    #                     c_pools_fire_non_CO2,
                    #                     c_pools_no_fire, forest_dist_last, interval_end_year, c_dens_in,
                    #                     rf_post_dist, most_recent_year_not_tall_veg, Cf, Gef_ch4_forest, Gef_n2o_forest)
                ...
                # Sets the RF from this interval as the RF from the previous interval for the next interval
