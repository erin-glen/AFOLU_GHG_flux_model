...
Coiled small tests:
python -m src.utilities.create_cluster -n 1 -t 1 -m 32 -cn LULUCF_model_mangrove_testing
python -m src.LULUCF.scripts.core_model.0_calculate_LULUCF_fluxes -cn LULUCF_model_mangrove_testing -bb 116.25 -2.25 116.5 -2 -cs 0.25 -yr 2000 2020 --run_date 20250821
python -m src.LULUCF.scripts.core_model.0_calculate_LULUCF_fluxes -cn LULUCF_model -bb 115.25 -3.75 115.5 -3.5 -cs 0.25 -yr 2000 2023 --run_date 20250821
...
TODO: Test AWS credentials before running so they don't error at upload step.
- Could we add in a way to run from upload step if credentials error out?
...
#@jit(nopython=True)  #TODO: turn back on and comment out print statements
    ...
    for i, interval_end_year in enumerate(interval_end_years):
        ...
        for row in range(LC_curr_block.shape[0]):
            for col in range(LC_curr_block.shape[1]):
                ...
                short_veg_AGC_BGC_RF = np.array([short_veg_AGC_RF, short_veg_BGC_RF, 0.0, 0.0]).astype('float32')
                ...
                # If mangroves are present, we assume that there was were never other terrestrial forest type before gain
                # and that there will not be conversion to other terrestrial forest type after loss
                ...
                if is_ever_mang:
                    ...
                    # to determine if this interval is mangrove loss, gain, or maintenance or before gain or after loss.
                    # first_mang_gain_year = 0 if there is extent in first year of the timeseries
                    # last_mang_loss_year = 0 if there is extent in last year of the timeseries
                    ...
                    mang_interval_timeseries = mang_timeseries[mang_year_index_list]
                    print(f"mang_interval_timeseries between {interval_start_year} - {interval_end_year}: {mang_interval_timeseries}")
                    #TODO: Make sure its working correctly after 2020, through 2023
                    ...
                    #TODO: Can delete this after testing

                    # Determines whether this interval is mangrove gain, maintenance, permanent loss, before gain, or
                    # after permanent loss using the first year of mangrove gain and last year of mangrove loss
                    mang_loss, mang_gain, mang_remaining_mang, before_mang_gain, after_mang_loss = nu.mangrove_states(interval_start_year, interval_end_year, first_mang_gain_year, last_mang_loss_year)
                    #TODO: Make sure this works for 2000-2005 interval (i.e. before_mang_gain was defaulting to mang_remaining_mang before)
                    ...
                    # 2) mang_loss_year_in_interval: the first year there was mangrove loss during the interval [i.e. 2016]
                    # 3) mang_gain_year_count_pre_loss: number of sequestration years in interval before mangrove loss
                    # 4) mang_gain_year_count_post_loss: number of sequestration years in interval after mangrove loss
                    # If there are multiple years of loss (1->0) in the interval, the first one is reported.
                    # If no loss, then mang_gain_year_count_pre_loss = number of years with mangrove extent during
                    # the interval and mang_gain_year_count_post_loss = 0
                    mang_loss_in_interval, mang_loss_year_in_interval, mang_gain_year_count_pre_loss, mang_gain_year_count_post_loss = nu.mangrove_gain_year_count_summary(mang_interval_timeseries, interval_start_year, interval_end_year)
                    #TODO: Check that this works for before_mang_gain and after_mang_loss now
                    #TODO: Currently gets information for the first year of loss during interval.
                    # Should we change to using the last year of loss during the interval?
                    # Note: Currently could get mang_gain_year_count_post_loss > 0 for mang_loss
                    ...
                    print(f"before_mang_gain: {before_mang_gain}")
                    print(f"after_mang_loss: {after_mang_loss}")
                ...
                #todo: Q - gain_year_count, should gain year count be mang_gain_year_count_pre_loss, mang_gain_year_count_post_loss, or the sum?
                # currently mang_gain_year_count_pre_loss.
                ...
                if mang_gain:
                    ...
                    if mang_loss_in_interval:
                        ...
                        (c_gross_emis_out, c_gross_removals_out, c_dens_out, agc_ef_out_cell, gain_year_count, forest_age_end_of_interval) = (
                            nu.calc_mang_loss(interval_length, mang_c_pools_EF_no_fire, mang_loss_year_in_interval,
                                mang_gain_year_count_pre_loss,mang_gain_year_count_post_loss, RF_AGC_final, RF_BGC_final, interval_end_year,
                                c_dens_in_NT_T, most_recent_year_not_tall_veg, deadwood_c_ratio_mang, litter_c_ratio_mang))
                        ...
                        (c_gross_emis_out, c_gross_removals_out, c_dens_out, gain_year_count, forest_age_end_of_interval) = (
                            nu.calc_mang(0, mang_gain_year_count_pre_loss, mang_gain_year_count_post_loss,
                ...
                ### Mangrove loss
                    ...
                    node = nu.accrete_node(node, 1)
                    node = nu.accrete_node(node, 2)             # Permanent loss of mangroves (12)
                    RF_AGC_final = mangrove_AGC_RF
                    RF_BGC_final = RF_AGC_final * r_s_ratio_mang
                    ...
                    (c_gross_emis_out, c_gross_removals_out, c_dens_out, agc_ef_out_cell, gain_year_count, forest_age_end_of_interval) = (
                        nu.calc_mang_loss(interval_length, mang_c_pools_EF_no_fire, mang_loss_year_in_interval,
                            mang_gain_year_count_pre_loss, mang_gain_year_count_post_loss, RF_AGC_final, RF_BGC_final, interval_end_year,
                            c_dens_in, most_recent_year_not_tall_veg, deadwood_c_ratio_mang, litter_c_ratio_mang))

                    #todo: q - If we are assuming no mangrove conversion, there should be no difference in how this is calculated
                    # for cropland, builtup, or other. Do we need seperate nodes for these?
                    if water_LC_curr:
                ...
                ### Mangrove remaining mangrove
                    ...
                    if mang_loss_in_interval:
                        ...
                        (c_gross_emis_out, c_gross_removals_out, c_dens_out, gain_year_count, forest_age_end_of_interval) = (
                            nu.calc_mang(forest_age_start_of_interval, mang_gain_year_count_pre_loss, mang_gain_year_count_post_loss,
                ...
                ### Before mangrove gain or after mangrove loss
                elif before_mang_gain or after_mang_loss:
                    node = nu.accrete_node(node, 1)
                    node = nu.accrete_node(node, 0)

                    c_dens_out = c_dens_in_empty
                    gain_year_count = 0
                    forest_age_end_of_interval = 0

                    if before_mang_gain:
                        state_out = nu.accrete_node(node, 0)  # Mangrove before first mangrove gain (100)
                        print(f"Node code is {state_out}, mangrove before first mangrove gain (100)")

                    elif after_mang_loss:
                        state_out = nu.accrete_node(node, 1)  # Mangrove after permanent mangrove loss (101)
                        print(f"Node code is {state_out}, mangrove after permanent mangrove loss (101)")
                ...
                ### Tree loss
                elif tree_loss:  # Trees converted to non-trees (3)    ##TODO: @Mel If mangrove branch at top, no exception needed here?
                    ...
                    if all_planted_trees:  # Full loss of planted trees (31)
                        ...
                        if all_oil_palm:  # Full loss of oil palm (incl. SDPT) (311->3111/3112)
                            ...
                            if LC_curr == cn.cropland:  # Full loss of non-oil palm planted trees as cropland (3121)
                                node = nu.accrete_node(node, 1)
                                if planted_forest_tree_crop_cell == 2:  # Full loss of non-oil palm tree crops as cropland (31211->312111/312112)
                                    node = nu.accrete_node(node, 1)
                                    ...
                                    (state_out, c_gross_emis_out, c_gross_removals_out, non_co2_flux_out, c_dens_out,
                                     RF_AGC_final, RF_BGC_final, agc_ef_out_cell, gain_year_count, forest_age_end_of_interval) = nu.calc_T_NT(
                                        ...
                                        rf_post_dist, most_recent_year_not_tall_veg, Cf_forest, Gef_ch4_forest, Gef_n2o_forest,
                                        deadwood_c_ratio=0, litter_c_ratio=0)
                                else:  # Full loss of non-oil palm planted forest as cropland (31212->312121/312122)
                                    node = nu.accrete_node(node, 2)
                                    agc_rf_in = planted_forest_AGC_RF_cell  # 5-year intervals only
                                    bgc_rf_in = planted_forest_BGC_RF_cell  # 5-year intervals only
                                    c_pools_EF_fire_CO2 = cn.biomass_emissions_only
                                ...
                                if planted_forest_tree_crop_cell == 2:  # Full loss of non-oil palm tree crops as short vegetation (31221->312211/312212)
                                    ...
                                    c_pools_EF_fire_non_CO2 = cn.biomass_emissions_only
                                    c_pools_EF_no_fire = cn.biomass_emissions_only
                                    rf_post_dist = short_veg_AGC_BGC_RF
                                    (state_out, c_gross_emis_out, c_gross_removals_out, non_co2_flux_out, c_dens_out,
                                     RF_AGC_final, RF_BGC_final, agc_ef_out_cell, gain_year_count, forest_age_end_of_interval) = nu.calc_T_NT(
                                        node, interval_length, burned_in_curr_interval, agc_rf_in, bgc_rf_in, c_pools_EF_fire_CO2, c_pools_EF_fire_non_CO2,
                                        c_pools_EF_no_fire, first_year_annual_dist_during_interval, interval_end_year, c_dens_in,
                                        rf_post_dist, most_recent_year_not_tall_veg, Cf_forest, Gef_ch4_forest, Gef_n2o_forest,
                                        deadwood_c_ratio=0, litter_c_ratio=0)
                                else:  # Full loss of non-oil palm planted forest as short vegetation (31222->312221/312222)
                                    node = nu.accrete_node(node, 2)
                                    agc_rf_in = planted_forest_AGC_RF_cell  # 5-year intervals only
                                    bgc_rf_in = planted_forest_BGC_RF_cell  # 5-year intervals only
                                    c_pools_EF_fire_CO2 = cn.biomass_emissions_only
                                ...
                                node = nu.accrete_node(node, 3)
                                if planted_forest_tree_crop_cell == 2:  # Full loss of non-oil palm tree crops to settlement (31231->312311/312312)
                                    node = nu.accrete_node(node, 1)
                                    agc_rf_in = planted_forest_AGC_RF_cell  # 5-year intervals only
                                    bgc_rf_in = planted_forest_BGC_RF_cell  # 5-year intervals only
                                    c_pools_EF_fire_CO2 = cn.biomass_emissions_only
                                    ...
                                    (state_out, c_gross_emis_out, c_gross_removals_out, non_co2_flux_out, c_dens_out,
                                     RF_AGC_final, RF_BGC_final, agc_ef_out_cell, gain_year_count, forest_age_end_of_interval) = nu.calc_T_NT(
                                        ...
                                        rf_post_dist, most_recent_year_not_tall_veg, Cf_forest, Gef_ch4_forest, Gef_n2o_forest,
                                        deadwood_c_ratio=0, litter_c_ratio=0)
                                else:  # Full loss of non-oil palm planted forest to settlement (31232->312321/312322)
                                    node = nu.accrete_node(node, 2)
                                    agc_rf_in = planted_forest_AGC_RF_cell  # 5-year intervals only
                                    bgc_rf_in = planted_forest_BGC_RF_cell  # 5-year intervals only
                                    c_pools_EF_fire_CO2 = cn.biomass_emissions_only
                                ...
                                if planted_forest_tree_crop_cell == 2:  # Full loss of non-oil palm tree crops to anything else (31241->312411/312412)
                                    node = nu.accrete_node(node, 1)
                                    agc_rf_in = planted_forest_AGC_RF_cell  # 5-year intervals only
                                    bgc_rf_in = planted_forest_BGC_RF_cell  # 5-year intervals only
                                    c_pools_EF_fire_CO2 = cn.biomass_emissions_only
                                    ...
                                    (state_out, c_gross_emis_out, c_gross_removals_out, non_co2_flux_out, c_dens_out,
                                     RF_AGC_final, RF_BGC_final, agc_ef_out_cell, gain_year_count, forest_age_end_of_interval) = nu.calc_T_NT(
                                        ...
                                        rf_post_dist, most_recent_year_not_tall_veg, Cf_forest, Gef_ch4_forest, Gef_n2o_forest,
                                        deadwood_c_ratio=0, litter_c_ratio=0)
                                else:  # Full loss of non-oil palm planted forest to anything else (31242->312421/312422)
                                    node = nu.accrete_node(node, 2)
                                    agc_rf_in = planted_forest_AGC_RF_cell  # 5-year intervals only
                                    bgc_rf_in = planted_forest_BGC_RF_cell  # 5-year intervals only
                                    c_pools_EF_fire_CO2 = cn.biomass_emissions_only
                        ...
                        if part_or_full_dist_in_curr_interval:  # Trees partially disturbed in the current interval (421)
                            ...
                            if all_planted_trees:   # Planted trees partially disturbed in the current interval (4211)
                                ...
                                if sig_height_gain_prev_curr_abs:  # Oil palm/planted trees partially disturbed in the current interval with signif. height increase after (42111)
                                    ...
                                    else: # Planted trees partially disturbed in the current interval with signif. height increase after (421112->4211121/4211122)
                                        node = nu.accrete_node(node, 2)
                                    ...
                                    node = nu.accrete_node(node, 2)
                                    # Calculation function only uses the RFs for 5-year intervals but assigning them regardless of interval type for consistency
                                    if all_oil_palm: # Oil palm partially disturbed in the current interval without signif. height increase after (421121->4211211/4211212)
                                        node = nu.accrete_node(node, 1)
                                        agc_rf_in = cn.oil_palm_agc_rf
                                        bgc_rf_in = cn.oil_palm_bgc_rf
                                        c_pools_EF_fire_CO2 = cn.agc_emissions_only
                                        ...
                                        (state_out, c_gross_emis_out, c_gross_removals_out, non_co2_flux_out, c_dens_out,
                                         RF_AGC_final, RF_BGC_final, agc_ef_out_cell, gain_year_count, forest_age_end_of_interval) = nu.calc_T_T_non_stand_disturbs(
                                            ...
                                            rf_post_dist, most_recent_year_not_tall_veg,
                                            Cf_forest, Gef_co2_forest, Gef_ch4_forest, Gef_n2o_forest, deadwood_c_ratio=0, litter_c_ratio=0)
                                    else: # Planted trees partially disturbed in the current interval without signif. height increase after (421122->4211221/4211222)
                                        node = nu.accrete_node(node, 2)
                                        agc_rf_in = planted_forest_AGC_RF_cell
                                        bgc_rf_in = planted_forest_BGC_RF_cell
                                        c_pools_EF_fire_CO2 = cn.agc_emissions_only
                                ...
                                if tall_veg_LC_curr:  # Forest partially disturbed in the current interval (42121)
                                    node = nu.accrete_node(node, 1)
                                    if sig_height_gain_prev_curr_abs:  # Forest partially disturbed in the current interval with signif. height increase after (421211->4212111/4212112)
                                        # NOTE: This should only occur with 5-year interval data, not annual data.
                                        node = nu.accrete_node(node, 1)
                                        # Calculation function only uses the RFs for 5-year intervals but assigning them regardless of interval type for consistency
                                        agc_rf_in = natrl_forest_age_dependent_agc_rf
                                    ...
                                    else:  # Forest partially disturbed in the current interval without signif. height increase after (421212->4212121/4212122)
                                        # NOTE: All annual interval data is expected to use this branch.
                                        node = nu.accrete_node(node, 2)
                                        # Calculation function only uses the RFs for 5-year intervals but assigning them regardless of interval type for consistency
                                        agc_rf_in = natrl_forest_age_dependent_agc_rf
                                    ...
                                    node = nu.accrete_node(node, 2)
                                    if sig_height_gain_prev_curr_abs:  # Trees outside forests partially disturbed in the current interval with signif. height increase after (421221->4212211/4212212)
                                        # NOTE: This should only occur with 5-year interval data, not annual data.
                                        node = nu.accrete_node(node, 1)
                                        # Calculation function only uses the RFs for 5-year intervals but assigning them regardless of interval type for consistency
                                        ...
                                        (state_out, c_gross_emis_out, c_gross_removals_out, non_co2_flux_out, c_dens_out,
                                         RF_AGC_final, RF_BGC_final, agc_ef_out_cell, gain_year_count, forest_age_end_of_interval) = nu.calc_T_T_non_stand_disturbs(
                                            ...
                                            Cf_forest, Gef_co2_forest, Gef_ch4_forest, Gef_n2o_forest,
                                            deadwood_c_ratio=0, litter_c_ratio=0)
                                    else:  # Trees outside forests partially disturbed in the current interval without signif. height increase after (421222->4212221/4212222)
                                        # NOTE: All annual interval data is expected to use this branch.
                                        node = nu.accrete_node(node, 2)
                                        # Calculation function only uses the RFs for 5-year intervals but assigning them regardless of interval type for consistency
                                        agc_rf_in = cn.trees_outside_forests_agc_rf_max
                            ...
                            if all_planted_trees:  # Oil palm/planted trees not disturbed in the current interval (4221)
                                node = nu.accrete_node(node, 1)
                                # Calculation function only uses the RFs for 5-year intervals but assigning them regardless of interval type for consistency
                                if all_oil_palm:  # Oil palm not disturbed in the current interval (42211->422111/422112)
                                    node = nu.accrete_node(node, 1)
                                    RF_AGC_final = cn.oil_palm_agc_rf
                                    RF_BGC_final = cn.oil_palm_bgc_rf
                                    c_pools_EF_fire_CO2 = cn.agc_emissions_only
                                    ...
                                    (state_out, c_gross_emis_out, c_gross_removals_out, agc_ef_out_cell,
                                     non_co2_flux_out, c_dens_out, gain_year_count, forest_age_end_of_interval) = nu.calc_T_T_no_disturbs(
                                        ...
                                        Cf_forest, Gef_co2_forest, Gef_ch4_forest, Gef_n2o_forest, deadwood_c_ratio=0, litter_c_ratio=0)
                                else: # Planted trees not disturbed in the current interval (42212->422121/422122)
                                    node = nu.accrete_node(node, 2)
                                    RF_AGC_final = planted_forest_AGC_RF_cell
                                    RF_BGC_final = planted_forest_BGC_RF_cell
                                    c_pools_EF_fire_CO2 = cn.agc_emissions_only
                                ...
                                if tall_veg_LC_curr:  # Natural forest not disturbed in last interval (42221)
                                    node = nu.accrete_node(node, 1)
                                    if (most_recent_year_not_tall_veg > 0) or (part_or_full_dist_in_earlier_intervals > 0):  # Young secondary natural forest (422211->4222111/4222112)
                                        node = nu.accrete_node(node, 1)
                                        ...
                                        if primary_forest_proxy:  # Primary forest undisturbed since model start (4222121->42221211/42221212)
                                            ...
                                            (state_out, c_gross_emis_out, c_gross_removals_out, agc_ef_out_cell,
                                             non_co2_flux_out, c_dens_out, gain_year_count, forest_age_end_of_interval) = nu.calc_T_T_no_disturbs(
                                                ...
                                                Cf_forest, Gef_co2_forest, Gef_ch4_forest, Gef_n2o_forest,
                                                deadwood_c_ratio=deadwood_c_ratio_non_mang, litter_c_ratio=litter_c_ratio_non_mang)
                                else:  # Trees outside forests not disturbed in the current interval (42222->422221/422222)
                                    node = nu.accrete_node(node, 2)
                                    RF_AGC_final = cn.trees_outside_forests_agc_rf_max
                                    RF_BGC_final = RF_AGC_final * r_s_ratio_non_mang
                                    c_pools_EF_fire_CO2 = cn.agc_emissions_only
                ...
                ### Cropland converted to non-cropland (without trees)
                    ...
                    if short_veg_LC_curr:
                        node = nu.accrete_node(node, 1)  # Annual cropland converted to short vegetation (521->5211/5212)
