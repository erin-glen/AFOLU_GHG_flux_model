...
def connect_to_cluster(
    *,
    ...
    software: str = "wri-afolu-2024-05-01",
) -> Tuple[coiled.Cluster | None, Client]:

    if run_local:
        print("Local Dask client")
        return None, Client(processes=False)

    name = cluster_name or f"afolu_{timestr()}"
    ...
    cluster.adapt(min_workers=min_workers, max_workers=max_workers)
...
# ----------------------------------------------------------------------
_S3FS = None  # kept only so the name exists elsewhere
