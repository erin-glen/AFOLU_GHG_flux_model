...
starting at per-pixel outputs (no drainage model step).
...
Usage (examples):
  python -m src.scripts.core_model.run_gpd_gfw_co2_n2o
  python run_standard_500m_pipeline_drained_only.py --dry-run
  python run_standard_500m_pipeline_drained_only.py --continue-on-error --skip 02_run_zonal_stats[gpd]
  python run_standard_500m_pipeline_drained_only.py --shutdown-cluster
"""
...
DEFAULT_LOG_DIR = Path(lop.pipeline_log_dir("gpd_gfw_co2_n2o"))
...
def build_commands(
    ...
    # NOTE: We intentionally SKIP the drainage emissions model (step 0).
    ...
    cmds.append(
        ("01_build_zarr_caches[gfw]", base_zarr + ["--run_name", "gfw_standard_model_500m"])
    )
    cmds.append(
    ...
    cmds.append(
        ("02_run_zonal_stats[gfw]", base_zstats + ["--run_name", "gfw_standard_model_500m"])
    )
    cmds.append(
        ("02_run_zonal_stats[gpd]", base_zstats + ["--run_name", "gpd_standard_model_500m"])
    )
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(
        description="Run GFW+GPD 500m pipeline (no drainage step) on one Coiled cluster."
    )
    p.add_argument(
        "--output-date",
        default="20251120",
        help="YYYYMMDD for per-pixel + aggregate steps.",
    )
    p.add_argument(
        "--run-date",
        default="20251120",
        help="YYYYMMDD for zonal-statistics steps.",
    )
    p.add_argument(
        "--interval-end-years",
        default="2024",
        help="Interval end year(s) for zonal-statistics.",
    )
    p.add_argument(
        "--model-version",
        default="0_9_7",
        help="Model version for zonal-statistics.",
    )
    p.add_argument(
        "--log-dir",
        type=Path,
        default=DEFAULT_LOG_DIR,
        help="Directory for per-step logs.",
    )
    p.add_argument(
        "--dry-run",
        action="store_true",
    ...
    p.add_argument(
        ...
        nargs="*",
        default=[],
        help="List of step labels to skip (substring match).",
    )
    p.add_argument(
        "--no-cluster",
        action="store_true",
        help="Do not create/attach a Coiled cluster explicitly.",
    )
    p.add_argument(
        "--scheduler-address",
        default=None,
        help="Override DASK_SCHEDULER_ADDRESS for all steps.",
    )
    p.add_argument(
        "--shutdown-cluster",
        action="store_true",
        help="Close the Coiled cluster on exit.",
    )
