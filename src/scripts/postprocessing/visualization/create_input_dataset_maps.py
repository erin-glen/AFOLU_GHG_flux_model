...
Sources may be local paths, exact S3 URIs, S3/local globs, model 10-degree tile
templates containing ``{tile_id}``, or keys from
``constants_and_names.get_dynamic_download_dict``. The latter keeps figure
inputs tied to the model's current source catalog.

Example
-------
python -m src.scripts.postprocessing.visualization.create_input_dataset_maps \
  --config docs/input_dataset_maps.example.json \
  --output-dir C:/tmp/afolu/input_dataset_maps
...
def validate_config(config: Mapping[str, Any]) -> TargetGrid:
    ...
    for index, dataset in enumerate(datasets, start=1):
        ...
        model_input = str(dataset.get("model_input", "")).strip()
        if bool(sources) == bool(model_input):
            raise ValueError(
                f"Dataset {name!r} must define exactly one of source or model_input"
            )
        if model_input and not isinstance(config.get("model_context"), Mapping):
            raise ValueError(
                f"Dataset {name!r} uses model_input, so top-level model_context is required"
            )
    ...
    if any(str(item.get("model_input", "")).strip() for item in datasets):
        context = config["model_context"]
        try:
            start_year = int(context["interval_start_year"])
        except (KeyError, TypeError, ValueError) as exc:
            raise ValueError("model_context.interval_start_year is required and must be an integer") from exc
        try:
            end_year = int(context.get("interval_end_year", start_year))
        except (TypeError, ValueError) as exc:
            raise ValueError("model_context.interval_end_year must be an integer") from exc
        if end_year < start_year:
            raise ValueError("model_context.interval_end_year must be >= interval_start_year")
...
def _resolve_model_sources(
    ...
    """Resolve a model input through the repo's canonical path dictionary."""
    ...
    start_year = int(context["interval_start_year"])
    end_year = int(context.get("interval_end_year", start_year))
    peat_dataset = str(context.get("peat_dataset", "ogh"))
    key = str(dataset["model_input"])
    paths: list[str] = []
    for tile_id in tile_ids:
        ...
        if key not in inputs:
            ...
            raise ValueError(
                f"Unknown model_input {key!r}; available keys for this context: {available}"
            )
...
def resolve_dataset_sources(
    ...
    *,
    ...
    if dataset.get("model_input"):
        raw_paths = _resolve_model_sources(dataset, config, tile_ids)
    else:
        raw_paths = []
    ...
    *,
    ...
    allow_missing = bool(
        dataset.get(
            ...
            or bool(dataset.get("model_input")),
        )
    )
    ...
    *,
    ...
    for index, (dataset, sources, output_paths) in enumerate(
        ...
        record = {
            ...
            "source_mode": "model_input" if dataset.get("model_input") else "explicit_source",
            "model_input": dataset.get("model_input"),
