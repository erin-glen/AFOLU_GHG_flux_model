...
def get_input_datasets(pixel_resolution: str = "8000_pixels", date: str = today_date) -> list:
    """Return list with the S3 folder for SDPT raster chunks."""
    return [
        posixpath.join(
            full_bucket_prefix,
            pcn.datasets["sdpt"]["s3_processed_base"],
            pixel_resolution,
            date,
        )
    ]
...
def main(
    ...
    stage = f"sdpt_preprocessing__outputs_aggregated_to_10x10deg_{pixel_resolution}_{date}"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Aggregate SDPT raster outputs to 10x10 degree geotifs."
    )
    ...
    parser.add_argument(
        ...
        "--date",
        default=today_date,
        help="Date (YYYYMMDD) of SDPT outputs",
    )
