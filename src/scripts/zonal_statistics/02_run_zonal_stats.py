...
def decode_emissions_state_to_legacy(
    ...
    drained = drained.rename("drained_state_nodes").assign_coords(emissions_nodes_aligned.coords).transpose(*emissions_nodes_aligned.dims)
    burned = burned.rename("burned_state_nodes").assign_coords(emissions_nodes_aligned.coords).transpose(*emissions_nodes_aligned.dims)
...
def decode_emissions_state_branch(
    ...
    *,
    ...
    name = "drained_state_nodes" if branch == "drained" else "burned_state_nodes"
    return decoded.rename(name).assign_coords(emissions_nodes_aligned.coords).transpose(*emissions_nodes_aligned.dims)
        ...
        for interval_start_year, interval_end_year in interval_pairs:
            ...
            if need_drained:
                ...
                with dask.annotate(label=f"reduce:drained:{interval}"):
                    ...
                    area_layer_d = drop_scalar_year_coord(ref if ref.dtype == np.float32 else ref.astype("float32"))
                    first_flux_d = zarr_data[drained_fluxes[0]]
                    logger.info(
                        "Drained pre-concat layer diagnostics: interval=%s flux_dims=%s flux_coords=%s flux_dtype=%s area_dims=%s area_coords=%s area_dtype=%s",
                        interval,
                        first_flux_d.dims,
                        list(first_flux_d.coords),
                        first_flux_d.dtype,
                        area_layer_d.dims,
            ...
            if need_burned:
                ...
                with dask.annotate(label=f"reduce:burned:{interval}"):
                    ...
                    area_layer_b = drop_scalar_year_coord(ref if ref.dtype == np.float32 else ref.astype("float32"))
                    first_flux_b = zarr_data[burned_fluxes[0]]
                    logger.info(
                        "Burned pre-concat layer diagnostics: interval=%s flux_dims=%s flux_coords=%s flux_dtype=%s area_dims=%s area_coords=%s area_dtype=%s",
                        interval,
                        first_flux_b.dims,
                        list(first_flux_b.coords),
                        first_flux_b.dtype,
                        area_layer_b.dims,
