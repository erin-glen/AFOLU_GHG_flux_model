...
def calculate_drainage_and_emissions(in_dict_uint8, in_dict_int16, in_dict_float32):
    ...
    The function returns two dictionaries:
      - out_dict_uint32: includes 'soil' and 'state' arrays (uint32),
      - out_dict_float32: includes multiple emission arrays (float32).

    Args:
        in_dict_uint8 (Dict[str, uint8[:, :]]): Dictionary of uint8 input arrays (e.g. peat, land_cover, burned_area).
        in_dict_int16 (Dict[str, int16[:, :]]): Dictionary of int16 input arrays (e.g. climate_domain).
        in_dict_float32 (Dict[str, float32[:, :]]): Dictionary of float32 input arrays (e.g. dadap, osm_roads).

    Returns:
        out_dict_uint32 (Dict[str, uint32[:, :]]): Dictionary of uint32 output arrays.
        out_dict_float32 (Dict[str, float32[:, :]]): Dictionary of float32 output arrays.
    """
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            # --------------------------------------------------
            #  A) Drainage logic (soil_block = 0, 1, or 2)
            # --------------------------------------------------
            if peat == 1:
                # We have peat
            ...
            # Store the drainage node (for debug / classification)
            state_out_block[row, col] = node

            # B) If drained peat => fill drainage EFs
            if soil_block[row, col] == 2:
                node = nu.accrete_node(node, 1)

                # Offsite defaults by ecozone
                ...
                (co2_emissions,
                 ...
                 co2_offsite_emissions) = nu.calculate_emissions_co2e(
                     ef_co2,
                     ef_n2o,
                     ef_ch4_land,
                     ef_ch4_ditch,
                ...
                # Not drained or not peat => no drainage emissions
                node = nu.accrete_node(node, 2)
                state_out_block[row, col] = node
            ...
            # -----------------------------------------------------
            #  C) Burned-area logic for peat (drained or undrained).
            # -----------------------------------------------------
                ...
                if burned_val > 0 and soil_block[row, col] in (1, 2):
                    ...
                    if ecozone == boreal_code:
                        ...
                        if soil_block[row, col] == 2:
                            ...
                            # If undrained peat is tropical, the code zeros out. (Example rule)
                    ...
                    # Accrete the burned_node into the main state
                    state_out_block[row, col] = nu.accrete_node(state_out_block[row, col], burned_node)

                else:
                    # No burn or not peat => zero out burned outputs
    ...
    # Add burned emissions (including total CO₂e) to typed dict
