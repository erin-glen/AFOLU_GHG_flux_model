...
def zarr_1x1_deg_stats(bounds, var_name, year_idx, zarr_path):
    ...
    # lu.print_and_log(f"Getting stats for {var_name} for year {year_idx} for {bounds}: {timestr()}", False, logger_worker)
    # start_time = time.time()

    bounds_str = boundstr(bounds)  # String form of chunk bounds, from e.g., [8, -1, 9, 0] to 8_-1_9_0
    ...
    zarr_stats_raw = calculate_stats(zarr_chunk_array, pattern_with_units, bounds_str, tile_id, 'zarr_stats')

    # end_time = time.time()
    # lu.print_and_log(f"  Calculated stats for {pattern_with_units} for {year} for {bounds} in {round(end_time - start_time)} seconds: {timestr()}", False, logger_worker)
...
def compare_dataset_year_chunk_stats(all_merged_tables, chunk_stats_variable_year_zarr, main_logger,
    ...
    main_logger.info(f"    Subsetting zarr table to numeric columns for {var_name} for {year}: {timestr()}")
    ...
    main_logger.info(f"    Renaming zarr columns for {var_name} for {year}: {timestr()}")
    ...
    main_logger.info(f"    Converting zarr columns to numeric for {var_name} for {year}: {timestr()}")
    ...
    main_logger.info(f"    Merging zarr data to original model data for {var_name} for {year}: {timestr()}")
    ...
    main_logger.info(f"    Calculating differences for {var_name} for {year}: {timestr()}")
