...
### Core model and aggregation are dual-writing `combined_state`
- The drainage model now packs a unified state raster into `outputs["combined_state"]`, derived from drained + burned state rasters.
- Aggregation includes `combined_state` in its output type list even if constants lag.
...
- `02_run_zonal_stats.py` includes `combined_state_nodes` in `DATASETS`.
- If `drained_state` and/or `burned_state` are missing from mega-zarr, it falls back to decoding from `combined_state` using `unpack_combined_state_to_legacy`.
...
There is no first-class `by_combined_state_*` output yet.

**Impact:** you can *run* zonal stats from `combined_state`, but downstream QA/QC still lacks a canonical unified-state artifact to validate transition quality directly.

## Recommended path forward

### Phase 1 (safe, immediate): keep compatibility outputs, add explicit unified QA artifacts
1. **Naming bridge:** keep wire format `combined_state` but introduce alias language `combined_state` in docs/logs/CLI help so users can discover intent without breaking existing runs.
2. **Zonal outputs:** add optional emissions-state parquet outputs:
   - `by_adm0__combined_state__flux_type__interval_end`
   - `by_adm0__combined_state__area__interval_end` (if you keep area separate)
...
1. **Decode parity check in zonal run:** when both direct legacy rasters and unified raster are present, sample-check or full-check that:
   - `decode(combined_state).drained == drained_state`
   - `decode(combined_state).burned == burned_state`
...
1. Switch dashboard/consumer defaults to `by_combined_state_*`.
...
1. **Extend `pub_common.py` SQL builders** with `table_by_combined_state_sql()` and country variant.
2. **In `pub_assets.py`**, register a new `combined_state_ctx` lookup sourced from `zonal_constants` unified mapping (or deterministic decode + join strategy).
3. **In `pub_assets.py` writer block**, emit the two new `by_combined_state_*` CSVs in parallel with existing exports.
...
- `by_combined_state_period.csv` and country variant are generated in `pub_assets` output.
